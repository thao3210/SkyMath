...
const NoteworthyBlogs = () => {
    ...
    return (
        <div className="py-3">
            <h4 className="fw-bold border-bottom border-2 pb-3">Tin đáng chú ý</h4>
            <div className="d-flex gap-3 flex-wrap">
                {blogsData.map((blog, index) => (
                    <div key={index} className="card card-blog rounded-0 border-0">
                        <img src={blog.imageLink} className="card-img-top rounded-0" alt={blog.title} />
                        <div className="py-lg-2">
                            ...
                            <Link to={`/NewsAndEventDetail`} className="card-title fw-bold short-content-blog card-blog-title">{blog.title}</Link>
                        </div>
                    </div>
                ))}
            </div>
...
export default NoteworthyBlogs;